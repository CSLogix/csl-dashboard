"""Patch app.py to add clickable team names and dedicated rep dashboards."""
...
# 1. Make team member names clickable — wrap name in <a> link
old_team_name = '''<div class="team-name"><span>{rep}</span><span class="team-count" style="color:{info['color']}">{d['loads']} Loads</span></div>'''

new_team_name = '''<div class="team-name"><span><a href="/rep/{rep}" style="color:var(--text-primary);text-decoration:none;" onmouseover="this.style.color='{info['color']}'" onmouseout="this.style.color='var(--text-primary)'">{rep}</a></span><span class="team-count" style="color:{info['color']}">{d['loads']} Loads</span></div>'''

code = code.replace(old_team_name, new_team_name)

# 2. Add the /rep/{name} route before the @app.on_event("startup")
rep_route = '''
# ---------------------------------------------------------------------------
# Rep Dashboard
# ---------------------------------------------------------------------------
...
print('app.py patched with rep dashboard')
