...
/**
 * Render a slide-over panel that displays detailed information, tracking, documents, emails, notes,
 * driver contact, and financial tools for a selected shipment, with inline editing and action handlers.
 *
 * @param {Object|null} selectedShipment - The currently selected shipment object (or null to close the panel).
 * @param {Function} setSelectedShipment - Setter to update or clear the selected shipment.
 * @param {Array<Object>} shipments - Array of all shipments used for lookups and live values.
 * @param {Function} setShipments - Setter to update the shipments array (e.g., for local edits).
 * @param {Function} handleStatusUpdate - Callback(id, newStatus) to change the shipment's status.
 * @param {string|null} editField - Identifier of the field currently being edited in the slide-over.
 * @param {Function} setEditField - Setter to change which inline field is in edit mode.
 * @param {string} editValue - Current edit buffer value for the inline editor.
 * @param {Function} setEditValue - Setter for the inline edit buffer.
 * @param {Function} handleFieldEdit - Callback(id, field, value) for non-API inline field edits.
 * @param {Function} addSheetLog - Function(message) to append a change log entry for the sheet/dashboard.
 * @param {Array<Object>} carrierDirectory - Optional carrier directory used to display MC#/carrier lookups.
 * @param {Function} onDocChange - Optional callback invoked after documents are added/removed/reclassified.
 * @param {boolean} isMobile - True when rendering for a mobile viewport (adjusts layout/width).
 * @param {boolean} expandEmailsOnOpen - If true, auto-expands and scrolls the emails section when the panel opens.
 * @param {Function} onConsumeExpandEmails - Optional callback invoked after auto-expanding emails to clear the trigger.
 * @param {Function} handleFieldUpdate - Callback(shipment, field, value, opts) for server-backed field updates.
 * @param {Function} handleMetadataUpdate - Callback(shipment, key, value, opts) to persist metadata like rates/notes.
 * @param {Function} handleApplyRate - Callback(rateQuote, opts) to apply a suggested rate quote to the shipment.
 */
