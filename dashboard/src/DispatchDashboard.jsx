      ...
      {showAddForm && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, animation: "fade-in 0.2s ease" }}
          ...
          <div onClick={e => e.stopPropagation()} className="glass-strong" style={{ borderRadius: 20, padding: 28, width: 460, maxHeight: "85vh", overflow: "auto", animation: "slide-up 0.3s ease", border: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <div style={{ fontSize: 11, color: "#8B95A8", marginBottom: 20 }}>Creates row in dashboard + Google Sheet</div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div>
          ...
          <select value={form.account} onChange={e => set("account", e.target.value)} style={inputStyle}>
        ...
        <div>
          ...
          <select value={form.status} onChange={e => set("status", e.target.value)} style={inputStyle}>
            {(form.moveType === "FTL" ? FTL_STATUSES : STATUSES).filter(s => s.key !== "all").map(s => <option key={s.key} value={s.key} style={{ background: "#0D1119" }}>{s.label}</option>)}
          </select>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div><label style={labelStyle}>Carrier</label><input value={form.carrier} onChange={e => set("carrier", e.target.value)} placeholder="Carrier name" style={inputStyle} /></div>
        <div>
          <label style={labelStyle}>Move Type</label>
          <select value={form.moveType} onChange={e => set("moveType", e.target.value)} style={inputStyle}>
            {["Dray Import", "Dray Export", "FTL"].map(t => <option key={t} style={{ background: "#0D1119" }}>{t}</option>)}
          </select>
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
